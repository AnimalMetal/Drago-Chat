...
DEFAULT_CONFIG = {
    ...
    "messages_folder": os.path.join(os.path.expanduser("~"), "Drago Chat Messages"),
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    def createMenu(self):
        try:
            ...
            self.chatMenuItem = self.toolsMenu.Append(wx.ID_ANY, "Drago &Chat")
    ...
    @script(description="Connect", category="Drago Chat")
    ...
    def save_message_locally(self, chat_id, message):
        ...
        try:
            ...
            messages_folder = self.config.get('messages_folder', os.path.join(os.path.expanduser("~"), "Drago Chat Messages"))
    ...
    def check_for_updates(self, show_no_update=True):
        ...
        def check():
            try:
                ...
                log.info(f"Drago Chat: Reading manifest from {manifest_path}")
                ...
                if 'version' in manifest:
                    ...
                    log.warning("Drago Chat: Could not read version from configobj, trying direct read")
                ...
                log.info(f"Drago Chat: Current version: {current_version}")
                ...
                log.info(f"Drago Chat: Latest version: {latest_version}")
                ...
                error_msg = str(e)
                from logHandler import log
                log.error(f"Drago Chat: Update check error: {error_msg}")
                import traceback
    ...
    def download_update(self, download_url, version):
        ...
        def download():
            try:
                from logHandler import log
                log.info(f"Drago Chat UPDATE: Starting download from {download_url}")
                log.info(f"Drago Chat UPDATE: Python version: {sys.version}")
                ...
                log.info("Drago Chat UPDATE: Calling requests.get()")
                response = requests.get(download_url, timeout=30, allow_redirects=True, stream=False)
                
                log.info(f"Drago Chat UPDATE: Response status: {response.status_code}")
                log.info(f"Drago Chat UPDATE: Content-Type: {response.headers.get('Content-Type', 'unknown')}")
                log.info(f"Drago Chat UPDATE: Content-Length: {len(response.content)} bytes")
                ...
                if response.status_code == 200:
                    ...
                    log.info(f"Drago Chat UPDATE: File written successfully: {file_size} bytes")
                    log.info(f"Drago Chat UPDATE: File exists check: {os.path.exists(filepath)}")
                    
                    if not os.path.exists(filepath) or file_size == 0:
                        log.error("Drago Chat UPDATE: File verification failed!")
                        wx.CallAfter(lambda: ui.message(_("Download failed: File not saved")))
                        return
                    
                    ui.message(_("Download complete. Installing..."))
                    log.info("Drago Chat UPDATE: Calling install_update()")
                    ...
                    log.error(f"Drago Chat UPDATE: HTTP error {response.status_code}")
                    wx.CallAfter(lambda: ui.message(_("Download failed: Status {status}").format(status=response.status_code)))
            except Exception as e:
                from logHandler import log
                import traceback
                log.error(f"Drago Chat UPDATE: Download exception: {e}")
                log.error(traceback.format_exc())
    ...
    def install_update(self, filepath, version):
        ...
        try:
            from logHandler import log
            log.info(f"Drago Chat UPDATE: install_update called")
            log.info(f"Drago Chat UPDATE: File: {filepath}")
            log.info(f"Drago Chat UPDATE: File exists: {os.path.exists(filepath)}")
            ...
            file_size = os.path.getsize(filepath)
            log.info(f"Drago Chat UPDATE: File size: {file_size}")
            ...
            if result == wx.ID_YES:
                log.info("Drago Chat UPDATE: User confirmed, launching installer")
                ...
                try:
                    ...
                    log.info(f"Drago Chat UPDATE: NVDA executable: {nvda_exe}")
...
class ChatWindow(wx.Frame):
    def __init__(self, parent, plugin):
        super().__init__(parent, title=_("Drago Chat"), size=(800, 600))
        self.plugin = plugin
...
class SettingsDialog(wx.Dialog):
    def __init__(self, parent, plugin):
        ...
        self.messagesFolderText = wx.TextCtrl(generalPanel, value=plugin.config.get("messages_folder", os.path.join(os.path.expanduser("~"), "Drago Chat Messages")))
